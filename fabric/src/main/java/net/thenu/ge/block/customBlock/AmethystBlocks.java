...
public class AmethystBlocks {public static void registerAmethystBlocks() {GE.LOGGER.info("Registering Mod GEBlocks for " + GE.MOD_ID);}
    ...
    public static final Block CIRCULAR_AMETHYST_LAMP = registerBlock(
            "circular_amethyst_lamp", new RedstoneLampBlock(AbstractBlock.Settings.create().mapColor(MapColor.PURPLE).strength(1.5f).luminance(state -> state.get(RedstoneLampBlock.LIT) ? 12 : 0).sounds(BlockSoundGroup.AMETHYST_BLOCK)));
    public static final Block CROSSED_AMETHYST_LAMP = registerBlock(
            "crossed_amethyst_lamp", new RedstoneLampBlock(AbstractBlock.Settings.create().mapColor(MapColor.PURPLE).strength(1.5f).luminance(state -> state.get(RedstoneLampBlock.LIT) ? 12 : 0).sounds(BlockSoundGroup.AMETHYST_BLOCK)));
